...
FROM     (
                ...
                AND   sgs.picking_completed_date::DATE >= {StartDate}::DATE 
                AND   sgs.picking_completed_date::DATE <= {EndDate}::DATE +1
                AND    (
                              Substring(sgs.picking_completed_date::          varchar,12,2)
                                     || Substring(sgs.picking_completed_date::varchar,15,2))::integer /100 :: numeric < 23.59
                AND    substring(sgs.picking_completed_date::varchar,12,2)::integer >= 15 ) picked
